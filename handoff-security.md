...
- **Rate limiting** (defense-in-depth, raises score ~8 → ~9): add a per-IP throttle if abuse appears. Turnstile already stops bots.
...
| Item | State |
|------|-------|
| `submit-quote` function | ACTIVE, `verify_jwt = true` |
| `submit-review` function | ACTIVE, `verify_jwt = true` |
